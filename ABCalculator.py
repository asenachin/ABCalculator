...
def popup_window(n1, c1, n2, c2):
    ...
    txtOutput.insert(tk.END, '\t\t\t' 
        + 'Контрольная   Тестовая' + os.linesep)        
    txtOutput.insert(tk.END, '\t\t\t'
        + 'Группа        Группа' + os.linesep)
    txtOutput.insert(tk.END, '-----------------------------------------'
        + '-----' + os.linesep)
    ...
    p1 = c1/n1
    p2 = c2/n2
    txtOutput.insert(tk.END, 'Конверсия             ' 
        + num_percent(p1) + ' ' + num_percent(p2) + os.linesep)
    sigma1 = math.sqrt(p1*(1-p1)/n1)
    sigma2 = math.sqrt(p2*(1-p2)/n2)
    txtOutput.insert(tk.END, 'Стандартное отклонение' 
    + num_percent(sigma1) + ' ' + num_percent(sigma2) + os.linesep)
    txtOutput.insert(tk.END, '-----------------------------------------'
        + '-----' + os.linesep)
    ...
    if upper2_95 > 1:
        upper2_95 = 1
    txtOutput.insert(tk.END, '95% Возможный разброс ' 
        + os.linesep)
    txtOutput.insert(tk.END, '                   От ' 
        + num_percent(lower1_95)  + '\t'
        + num_percent(lower2_95) + os.linesep)
    txtOutput.insert(tk.END, '                   До ' 
    ...
    if upper2_99 > 1:
        upper2_99 = 1
    txtOutput.insert(tk.END, '99% Возможный разброс ' 
        + os.linesep)
    txtOutput.insert(tk.END, '                   От ' 
    ...
    txtOutput.insert(tk.END, 'Z = ' + "{:.7f}".format(z_score)
        + os.linesep)
    ...
    lblComment95 = tk.Label(window, text = '95% уверенность:', 
        font=('Helvetica', 10, 'bold'))
    lblComment95.place(x=25, y=25)    
    if confidence_95:
        lblResult95 = tk.Label(window, text = 'Да', 
        font=('Helvetica', 12, 'bold'), fg = '#008800')
        lblResult95.place(x=180, y=25)    
    else:
        lblResult95 = tk.Label(window, text = 'Нет', 
        font=('Helvetica', 12, 'bold'), fg = '#ff0000')
        lblResult95.place(x=180, y=25)
            
    lblComment99 = tk.Label(window, text = '99% уверенность:', 
        font=('Helvetica', 10, 'bold'))
    lblComment99.place(x=25, y=65)    
    if confidence_99:
        lblResult99 = tk.Label(window, text = 'Да', 
        font=('Helvetica', 12, 'bold'), fg = '#008800')
    ...
    btnClosePopup = tk.Button(window, text="Закрыть", 
        font=('Helvetica', 10, 'bold'), command=window.destroy)
...
entVisitors1.insert(tk.END, '0') # 255
...
entConversions1.insert(tk.END, '0') # 26
...
entVisitors2.insert(tk.END, '0') # 235
...
entConversions2.insert(tk.END, '0') # 18
